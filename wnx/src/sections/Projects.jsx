            ...
            <Reveal as="li" className="card card--hover case" key={i} delay={i * 60}>
              {clean(item.image) && (
                <div className="case__media">
                  <img src={`${import.meta.env.BASE_URL}${item.image}`} alt={clean(item.imageAlt) || clean(item.title)} loading="lazy" />
                </div>
              )}
              <div className="case__head">
                <h3 className="case__title">
                  {clean(item.url) ? (
                    <a href={clean(item.url)} target="_blank" rel="noopener noreferrer">{clean(item.title)}</a>
                  ) : clean(item.title)}
                </h3>
